...
- Now if you compile and run the project, you will have Window as the following picture. First, the small sphere in **yellow** and **black** color are representing the **point light**, which emit light ray in 360 degree. Then, the two spheres in **yellow** and **white** color with arrows are representing the spot-light. The two planes in **yellow** and **blue** colors with many arrows are representing the area-light. The three big spheres in **blue**,**yellow**,and **red** colors are defined as the **sceneObject**. The rabbit, which is sitting on the **yellow** sphere is a Mesh object. Finally, the wall panels and the table panels are defined as **plane** object, which is inherited from the **SceneObject**.
