...
void RayTracingSystem::rayTracingDrawImage()
{
    ...
    for(int i=0;i<this->_imageWidth;i++)
    {
        for(int j=0;j<this->_imageHeight;j++)
        {
            ...
            {
                if(this->_scene_object_list.at(k)->intersect(temp_ray, temp_intersect_point, temp_normal))
                {
            ...
            if(is_ray_selected_object)
            {
                ...
                if(!temp_closest_object->isFloorPlane())
                {
                    ...
                    if(!temp_closest_object->getReflectable())
                    {
                        ...
                        temp_color += this->_integralThreeColor(temp_closest_point, temp_closest_normal, temp_closest_object->getDiffuseColor(), temp_closest_object->getSpecularColor());
                    ...
                    else
                    {
                        ...
                        temp_color += this->_integralThreeColor(temp_closest_point, temp_closest_normal, temp_closest_object->getDiffuseColor(), temp_closest_object->getSpecularColor());
                        temp_color += this->_integralThreeColor(temp_closest_point, temp_closest_normal, temp_diffus , temp_spec);
                        this->_image.setColor(i, _imageHeight - 1 -j, temp_color);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if(temp_closest_object->getFloorPlaneTexter()!=nullptr)
                        {
                            ...
                            temp_color += this->_integralThreeColor(temp_closest_point, temp_closest_normal, temp_diffuse_color_floor, ofColor::darkGray);
                            this->_image.setColor(i, _imageHeight - 1 -j, temp_color);
                        }
                        else
                        {
...
ofColor RayTracingSystem::_integralThreeColor(const glm::vec3 &point, const glm::vec3& normal, const ofColor diffuse, const ofColor specular)
{
